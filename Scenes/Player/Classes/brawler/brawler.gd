...
var attack3_max_t = 90
var attack4_max_t = 240
...
func attack_1():
	if Global.attack1t >= 1 and Global.attack1t <= 24:
		...
		if Global.attack1t == 16:
			...
			wave.set_damage(Global.power + (0.5 * Global.strength))
	...
	if Global.attack2t >= 1 and Global.attack2t <= 34:
		...
		if between(Global.attack2t, 30, 34) and Global.attack2t % 2 == 0:
			...
			if status.get_berserk():
				wave.set_fire(6)
			wave.set_damage((0.5 * Global.power) + (0.25 * Global.strength))
...
func attack_3():
	if Global.attack3t >= 1 and Global.attack3t <= 30:
		...
		if Global.attack3t == 15:
			wave = fist_wave.instantiate()
			wave.set_player(player)
			wave.set_damage((1 * Global.power) + (0.75 * Global.strength))
			wave.set_knockback(6)
...
func attack_4():
	if Global.attack4t >= 1 and Global.attack4t <= 60:
		...
		if between(Global.attack4t, 4, 60) and Global.attack4t % 4 == 0:
			...
			if status.get_berserk():
				wave.set_fire(1)
			wave.set_damage((0.1 * Global.power) + (0.1 * Global.strength))
...
func ultimate():
	if Global.ultimatet >= 1 and Global.ultimatet <= 60:
		...
		if Global.ultimatet == 10:
			...
			giant_fist.set_damage((6 * Global.power) + (4 * Global.strength))
