...
func ultimate():
	if Global.ultimatet >= 1 and Global.ultimatet <= 10:
		...
		if Global.ultimatet == 1:
			needle = needle_proj.instantiate()
			needle.set_player(player)
			needle.set_damage((2 * Global.power) + (1.5 * Global.dexterity))
